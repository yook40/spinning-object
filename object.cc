  ...
  while (1) {
    ...
    for (auto &pixel:object) {
      ...
      if (lum >= 0) {
        ...
        int lum_index = lum * (luminance.length() - 1) / sqrt(light_source[0] * light_source[0] +
    ...
    // compute and store the accurate coordinates and the surface normal of each pixel with type float
    for (auto &pixel:object) {
      float old_x = pixel.x;
      float old_y = pixel.y;
      float old_z = pixel.z;
